...
class SignupScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        username = w.InputBox(entry_frame, placeholder_text="What should we call you?", width=366, height=48)
        username.grid()
        ...
        w.Button(content_frame, text="Sign In", width=366, height=48, command=lambda: self.sign_in(username, password)).grid(pady=(27,0))
        #ctk.CTkButton(content_frame, text="Sign In", command=lambda: parent.show_screen(SignupScreen)).grid(pady=0,padx=0)
